...
public:
    virtual void onRecordPrivilegeChanged(bool bCanRec) override {
        if (bCanRec) {
            blog(LOG_INFO, "[Zoom to OBS] Telemetry: Recording Permission Granted!");
            ...
            if (ms && ms->GetMeetingRecordingController()) {
                ...
                if (g_videoRenderer && ms->GetMeetingParticipantsController()) {
                    g_videoRenderer->setRawDataResolution(ZOOM_SDK_NAMESPACE::ZoomSDKResolution_720P);
                    auto userList = ms->GetMeetingParticipantsController()->GetParticipantsList();
...
public:
    virtual void onMeetingStatusChanged(ZOOM_SDK_NAMESPACE::MeetingStatus status, int iResult = 0) override {
        if (status == ZOOM_SDK_NAMESPACE::MEETING_STATUS_INMEETING) {
            blog(LOG_INFO, "[Zoom to OBS] Telemetry: In Meeting Successfully!");
            ZOOM_SDK_NAMESPACE::IMeetingService* ms = nullptr;
            ZOOM_SDK_NAMESPACE::CreateMeetingService(&ms);
            if (ms && ms->GetMeetingRecordingController()) {
...
public:
    virtual void onAuthenticationReturn(ZOOM_SDK_NAMESPACE::AuthResult ret) override {
        blog(LOG_INFO, "[Zoom to OBS] Telemetry: Auth Return Result: %d", ret);
    ...
    virtual void onLoginReturnWithReason(ZOOM_SDK_NAMESPACE::LOGINSTATUS r, ZOOM_SDK_NAMESPACE::IAccountInfo* a, ZOOM_SDK_NAMESPACE::LoginFailReason re) override {}
...
public:
    ...
    ZoomSource(obs_source_t* src, const std::string& t) : source(src), type(t) {
        if (type == "Participant") {
            ...
            blog(LOG_INFO, "[Zoom to OBS] Telemetry: Participant Source Created.");
        }
    }
...
bool obs_module_load(void) {
    ...
    ZOOM_SDK_NAMESPACE::SDKError iErr = ZOOM_SDK_NAMESPACE::InitSDK(ip);
    blog(LOG_INFO, "[Zoom to OBS] Step 1: InitSDK Result: %d", iErr);

    if (iErr == ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS) {
        ZOOM_SDK_NAMESPACE::IAuthService* as = nullptr;
        ZOOM_SDK_NAMESPACE::SDKError sErr = ZOOM_SDK_NAMESPACE::CreateAuthService(&as);
        blog(LOG_INFO, "[Zoom to OBS] Step 2: CreateAuthService Result: %d", sErr);
